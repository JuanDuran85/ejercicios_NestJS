...
export class AggregateRehydrator {
  ...
  public async rehydrate<T extends VersionedAggregateRoot>(
    aggregateId: string,
    AggregateCls: Type<T>,
  ): Promise<T> {
    const events: SerializableEvent<any>[] =
      await this.eventStore.getEventsByStreamId(aggregateId);

    const AggregateClsWithDispatcher: Type<T> =
